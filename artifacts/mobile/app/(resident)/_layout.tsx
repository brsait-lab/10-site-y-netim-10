...
function NativeResidentTabs() {
  return (
    <NativeTabs>
      ...
      <NativeTabs.Trigger name="payments"><Icon sf={{ default: "creditcard", selected: "creditcard.fill" }} /><Label>Ödemeler</Label></NativeTabs.Trigger>
      <NativeTabs.Trigger name="merchants"><Icon sf={{ default: "bag", selected: "bag.fill" }} /><Label>Esnaflar</Label></NativeTabs.Trigger>
    ...
    <Tabs
      screenOptions={{
        ...
        tabBarBackground: () =>
          ...
          isWeb ? <View style={[StyleSheet.absoluteFill, { backgroundColor: colors.tabBar }]} /> : null,
        tabBarLabelStyle: { fontFamily: "Inter_500Medium", fontSize: 11, marginBottom: isWeb ? 0 : 4 },
      }}
    >
      <Tabs.Screen name="index" options={{ title: "Ana Sayfa", tabBarIcon: ({ color }) => isIOS ? <SymbolView name="house.fill" tintColor={color} size={22} /> : <Feather name="home" size={22} color={color} /> }} />
...
export default function ResidentLayout() {
  if (isLiquidGlassAvailable()) return <NativeResidentTabs />;
  return <ClassicResidentTabs />;
}
