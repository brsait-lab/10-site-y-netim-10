  ...
  return (
    <ScrollView style={[styles.root, { backgroundColor: colors.background }]} contentContainerStyle={[styles.scroll, { paddingTop: topPadding + 16, paddingBottom: insets.bottom + 100 }]} showsVerticalScrollIndicator={false}>
      <View style={styles.profileHeader}>
        <View style={[styles.avatarLarge, { backgroundColor: "#fef3c7", borderRadius: 40 }]}>
          <Text style={[styles.avatarText, { color: "#92400e" }]}>{user?.businessName?.[0]?.toUpperCase() || user?.name[0]?.toUpperCase()}</Text>
        </View>
        <Text style={[styles.userName, { color: colors.foreground }]}>{user?.businessName || user?.name}</Text>
        <View style={[styles.rolePill, { backgroundColor: "#fef3c7", borderRadius: 20 }]}>
          <Feather name="shopping-bag" size={12} color="#92400e" />
          <Text style={[styles.roleText, { color: "#92400e" }]}>Esnaf</Text>
        </View>
      </View>
      <View style={[styles.card, { backgroundColor: colors.card, borderRadius: colors.radius, borderColor: colors.border }]}>
        {[
          { icon: "user" as const, label: "Ad Soyad", value: user?.name || "-" },
          { icon: "briefcase" as const, label: "İşletme Adı", value: user?.businessName || "Belirtilmedi" },
          { icon: "tag" as const, label: "Kategori", value: user?.businessCategory || "Belirtilmedi" },
